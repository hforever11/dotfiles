...
| `Ctrl + h/j/k/l`   | ペイン移動 (tmux の `is_vim` 分岐を `~/.local/bin/herdr-navigate` で再現) |
| `Prefix + Ctrl + l` | クリアスクリーン (`C-l` を移動に奪った代わり。tmux と同じ) |
...
`herdr-navigate` は socket API で最前面プロセスを判定し、vim なら `C-h/j/k/l` を
ペインへ転送 (vim 内のウィンドウ移動を優先)、それ以外は `pane focus` でペインを移動する。
シェルの `C-l` (クリア) や fzf の `C-j/k` は潰れるが、これは tmux 時代と同じトレードオフ。

nvim 側は vim-tmux-navigator を廃止し、`keymaps.lua` の自前実装に置き換え済み:
ウィンドウ端で `C-h/j/k/l` を押すと `HERDR_PANE_ID` があれば `herdr pane focus`、
なければ tmux フォールバック時用に `tmux select-pane` を呼ぶ。
...
設定・バイナリとも tmux は残置 (`dot_config/tmux/`、Brewfile)。戻る場合は `tmux` を叩くだけ。
